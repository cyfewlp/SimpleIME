...
namespace Ime
{
auto ImGuiThemeLoader::GetAllThemeNames(const Path &path, std::vector<std::string> &themeNames, bool refresh) -> bool
{
...
void ImGuiThemeLoader::SetupMissingColors(ImGuiStyle &style)
{
    ...
    const auto     imColor = ImColor(
        static_cast<int>((color & 0xFF000000) >> 24), //
