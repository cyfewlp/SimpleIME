...
* [ ] keepimeopen <--- always enable ime

=== fix

* [ ] discard `E` key event when ime in input or wait input
* [ ] ImGui cursor not update when keep any key pressed
* [ ] detach, uninstall hooks when destory window

* [ ] Ctrl-c/v support
** In `DispatchInputEvent` detect copy command and transfer clipboard data to `TextService`
to disable game paste feature
* [ ] Support select candidate from UI
* [ ] Default Theme optimize?
* [x] Render current IME state: Is disable;
