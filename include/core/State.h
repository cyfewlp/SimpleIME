...
namespace Ime::Core
{
// \todo need refactor, may replace implement with bitset or just use a simple uint32_t, but this is more readable and easier to maintain.
class State
{
...
public:
    ...
    auto Clear(StateKey state) -> void { m_state.reset(state); }

    [[nodiscard]] auto Has(StateKey state) const -> bool { return m_state.all(state); }
    ...
    constexpr auto IsKeyboardOpen() const { return Has(KEYBOARD_OPEN); }

    constexpr auto IsImeInputting() const { return HasAny(IN_CAND_CHOOSING, IN_COMPOSING); }

    constexpr auto IsImeWaitingInput() const { return NotHas(IME_DISABLED, IN_ALPHANUMERIC); }
